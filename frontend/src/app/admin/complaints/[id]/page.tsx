          ...
          <div className="glass-card animate-fade-in-up" style={{ padding: 28 }}>
            ...
            {(complaint.vendorBeforeImageUrl || complaint.vendorAfterImageUrl) && (
              <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingBottom: 8, borderBottom: '1px solid var(--border-glass)' }}>
                  <h4 style={{ fontSize: 14, fontWeight: 700 }}>Verification Images</h4>
                  {complaint.status === 'RESOLVED' && (
                    <span className="badge badge-resolved">Pending Admin Verification</span>
                  )}
                </div>
                ...
                {complaint.status === 'RESOLVED' && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                    <button onClick={() => { setNewStatus('CLOSED'); setStatusComment('Images verified. Task completed.'); setShowStatusModal(true); }} className="btn btn-success btn-sm">
                      ✅ Verify & Close Task
                    </button>
                    <button onClick={() => { setNewStatus('IN_PROGRESS'); setStatusComment('Rework required based on images.'); setShowStatusModal(true); }} className="btn btn-ghost btn-sm" style={{ color: 'var(--accent-rose)' }}>
                      ❌ Reject & Rework
                    </button>
                  </div>
                )}
